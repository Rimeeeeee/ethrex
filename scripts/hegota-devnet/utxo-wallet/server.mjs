...
async function scanAuthenticated(request) {
  const { authenticatedScan } = await import('./authenticated.mjs');
  return authenticatedScan(request);
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/authenticated-discovery' && req.method === 'POST') return send(res, 200, await scanAuthenticated(await body(req)));
