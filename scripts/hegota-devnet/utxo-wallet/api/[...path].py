...
request_rpc = ContextVar("request_rpc", default=os.environ.get("UTXO_RPC", DEFAULT_RPC).strip())


def validate_rpc(candidate):
...
def signing_key(body):
    candidate = body.get("key")
    if not isinstance(candidate, str) or KEY_RE.fullmatch(candidate) is None:
        raise ValueError("private key must be exactly 32 bytes in hex; connect a wallet first")
    return candidate if candidate.startswith("0x") else "0x" + candidate
...
def forge(command: dict) -> dict:
    payload = json.dumps({"rpc": request_rpc.get(), **command}).encode()
    try:
...
def wallet_address(wallet_key: str) -> str:
...
def create_fresh_utxo(body: dict) -> dict:
    ...
    owner = wallet_address(wallet_key)
...
def handle_get(path: str, address=None):
    if path == "/api/status":
...
def handle_post(path: str, body: dict):
    if path == "/api/rpc":
        candidate = validate_rpc(body.get("url"))
        token = request_rpc.set(candidate)
        try:
            chain_id = rpc_call("eth_chainId", [])
        except Exception:
            raise ValueError("RPC URL did not respond")
        finally:
            request_rpc.reset(token)
        return {"rpc": candidate, "chainId": chain_id}

    if path == "/api/import":
        address = wallet_address(signing_key(body))
...
def handle_request(method, path, headers, body=None):
    token = request_rpc.set(validate_rpc(headers.get("x-utxo-rpc", request_rpc.get())))
    try:
        if method == "GET":
            return handle_get(path, headers.get("x-wallet-address"))
        return handle_post(path, body or {})
    finally:
        request_rpc.reset(token)
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            json_response(self, 200, handle_request("GET", urlparse(self.path).path, self.headers))
    ...
    def do_POST(self):
        try:
            ...
            json_response(self, 200, handle_request("POST", path, self.headers, read_json(self)))
        except FileNotFoundError as exc:
            error_response(self, exc, 404)
        except Exception as exc:
            error_response(self, exc)
