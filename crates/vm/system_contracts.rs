...
pub const INDEX_CONTRACT_RUNTIME_BYTECODE: [u8; 117] = [
    0x33, 0x73, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0xff, 0xfe, 0x14, 0x60, 0x60, 0x57, 0x60, 0x40, 0x36, 0x03, 0x60, 0x5c,
    0x57, 0x60, 0x20, 0x35, 0x80, 0x60, 0x80, 0x1c, 0x60, 0x5c, 0x57, 0x61, 0x04, 0x00, 0x81, 0x60,
    0x04, 0x81, 0x04, 0x43, 0x03, 0x04, 0x82, 0x82, 0x02, 0x92, 0x5f, 0x35, 0x81, 0x81, 0x06, 0x60,
    0x5c, 0x57, 0x04, 0x90, 0x81, 0x03, 0x19, 0x61, 0x03, 0xff, 0x10, 0x60, 0x5c, 0x57, 0x06, 0x01,
    0x54, 0x80, 0x15, 0x60, 0x5c, 0x57, 0x5f, 0x52, 0x60, 0x20, 0x5f, 0xf3, 0x5b, 0x5f, 0x5f, 0xfd,
    0x5b, 0x60, 0x40, 0x35, 0x60, 0x20, 0x35, 0x61, 0x04, 0x00, 0x81, 0x81, 0x02, 0x91, 0x5f, 0x35,
    0x04, 0x06, 0x01, 0x55, 0x00,
];
...
/// Devnet runtime bytecode of the EIP-8312 UTXO vault. It is derived from the
/// proposal runtime, with the UTXO index moved from log data to `topics[3]` for
/// the EIP-8304 + UPT integration profile.
///
...
/// `LOG4(topic, caller, recipient, index)` with `value` as data. A plain
/// transfer (empty calldata) reverts, so no value enters the vault without
/// creating a UTXO.
pub const UTXO_VAULT_RUNTIME_BYTECODE: [u8; 74] = [
    0x5f, 0x35, 0x60, 0x60, 0x1c, 0x80, 0x15, 0x60, 0x14, 0x36, 0x14, 0x15, 0x17, 0x34, 0x15, 0x17,
    0x60, 0x46, 0x57, 0x5f, 0x54, 0x80, 0x60, 0x01, 0x01, 0x5f, 0x55, 0x90, 0x34, 0x5f, 0x52, 0x33,
    0x7f, 0x3b, 0x19, 0x24, 0x14, 0x65, 0xa4, 0x7b, 0xc1, 0x87, 0xf1, 0xd9, 0xc7, 0xdb, 0x70, 0x83,
    0x48, 0x55, 0xa9, 0x07, 0x18, 0x37, 0x42, 0xa4, 0xb6, 0x3a, 0xa8, 0x24, 0xc5, 0x76, 0x29, 0x6f,
    0x5e, 0x60, 0x20, 0x5f, 0xa4, 0x00, 0x5b, 0x5f, 0x5f, 0xfd,
];
...
mod expiry_verifier_tests {
    ...
    #[test]
    fn index_contract_runtime_matches_published_assembly() {
        assert_eq!(INDEX_CONTRACT_RUNTIME_BYTECODE.len(), 117);
        assert_eq!(INDEX_CONTRACT_RUNTIME_BYTECODE[0x5c], 0x5b);
        assert_eq!(INDEX_CONTRACT_RUNTIME_BYTECODE[0x60], 0x5b);
        assert_eq!(
            hex::encode(INDEX_CONTRACT_RUNTIME_BYTECODE),
            concat!(
                "3373fffffffffffffffffffffffffffffffffffffffe1460605760403603605c57",
                "6020358060801c605c576104008160048104430304828202925f35818106605c",
                "5704908103196103ff10605c570601548015605c575f5260205ff35b5f5ffd5b",
                "604035602035610400818102915f350406015500"
            )
        );
    }
}
